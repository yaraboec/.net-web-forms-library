...
namespace UnitTests
{
    public class PublicationPlanTableTests
    {
        private static Guid ExistingId = Guid.NewGuid();
        private static Guid NonExistingId = Guid.NewGuid();

        private readonly PublicationPlanTable _entity = new PublicationPlanTable()
        {
            Id = ExistingId
        };

        private readonly PublicationPlanTable _nonExistingEntity = new PublicationPlanTable()
        {
            Id = NonExistingId
        };

        private readonly IEnumerable<PublicationPlanTable> _entities = new List<PublicationPlanTable>()
        {
            new PublicationPlanTable()
            {
                Id = ExistingId
            }
        };

        private Mock<IPublicationPlanTableRepository> _iSqlRepository;

        private PublicationPlanTableService _service;

        [SetUp]
        public void Setup()
        {
            _iSqlRepository = new Mock<IPublicationPlanTableRepository>();

            _service = new PublicationPlanTableService(_iSqlRepository.Object);
        }
        ...
        public void GetById_NonExistingId_ReturnsNull()
        {
            _iSqlRepository.Setup(x => x.GetById(ExistingId))
                .Returns((PublicationPlanTable)null);
